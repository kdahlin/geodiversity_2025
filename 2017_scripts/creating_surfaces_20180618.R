...
all.surf <- stack(stripe.surface, check.surface, continuous.surface, hill.surface,
                  gf.nug20.range100, gf.nug5.range500)

names(all.surf) <- c("stripe", "checker", "continuous", "hill", "gf_nug20_range100",
                     "gf_nug5_range500")

writeRaster(all.surf, "data/synthetic_surfaces_30m_20180615.tif")
...
all.surf.ag <- aggregate(all.surf, fact = 33.333, fun = mean, expand = TRUE, na.rm = TRUE)
all.surf.ag <- resample(all.surf.ag, empty.coarse.surf, method = "ngb")
writeRaster(all.surf.ag, "data/synthetic_surfaces_1000m_20180618.tif")
...
write.csv(sample.coords, "data/sampling_centroids_x12_20180309.csv", row.names = F)
